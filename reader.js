...
const el = {
  ...
  let state = { story: null, idx: 0, fs: 'fs-normal', dark: true, font: 'serif', lineSpacing: 1.95 };
  ...
  const el = {
    ...
    lineSelect: document.getElementById('lineSpacing'),
  ...
  function applyPreferences(){
    ...
    // font family
    if(el.contentArea){
      if(state.font === 'serif') el.contentArea.style.fontFamily = "Merriweather, Georgia, 'Times New Roman', serif";
      else el.contentArea.style.fontFamily = "Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial";
      el.contentArea.style.lineHeight = String(state.lineSpacing);
    }
    // sync selects if present
    if(el.fontSelect) el.fontSelect.value = state.font;
    if(el.lineSelect) el.lineSelect.value = String(state.lineSpacing);
  }
  ...
  function saveState(){ try{ localStorage.setItem('reader.state', JSON.stringify({idx:state.idx, fs:state.fs, dark:state.dark, font: state.font, lineSpacing: state.lineSpacing})); }catch{} }
  function loadState(){ try{ const s = JSON.parse(localStorage.getItem('reader.state')); if(s){ state.idx = (typeof s.idx === 'number') ? s.idx : 0; state.fs = s.fs||'fs-normal'; state.dark = !!s.dark; state.font = s.font || state.font; state.lineSpacing = s.lineSpacing || state.lineSpacing; } }catch{} }
  ...
  async function loadStory(){
    try{
      ...
      renderChapter();
      renderToc();
    }catch(err){ if(el.contentArea) el.contentArea.innerHTML = `<p style="color:#c00">Lỗi: ${err.message}</p>` }
  }

  /* TOC handling */
  function renderToc(){
    if(!state.story || !el.tocList) return;
  ...
  function highlightToc(){
    ...
    if(cur) cur.style.background = 'rgba(241,196,15,0.08)';
  }

  function openToc(){ if(el.tocModal) el.tocModal.setAttribute('aria-hidden','false'); }
  function closeToc(){ if(el.tocModal) el.tocModal.setAttribute('aria-hidden','true'); }

  /* Settings */
  function openSettings(){ if(el.settingsModal) el.settingsModal.setAttribute('aria-hidden','false'); }
  function closeSettings(){ if(el.settingsModal) el.settingsModal.setAttribute('aria-hidden','true'); }
  function resetPrefs(){ state.fs='fs-normal'; state.font='serif'; state.lineSpacing=1.95; state.dark=true; saveState(); applyPreferences(); }

  /* Progress */
  function updateProgress(){
    if(!el.progressBar || !el.contentArea) return;
    const max = el.contentArea.scrollHeight - el.contentArea.clientHeight;
    const pct = max > 0 ? Math.min(100, Math.round((el.contentArea.scrollTop / max) * 100)) : 0;
  ...
  if(el.fontSelect) el.fontSelect.addEventListener('change', (e)=>{ state.font = e.target.value; saveState(); applyPreferences(); });
  if(el.lineSelect) el.lineSelect.addEventListener('change', (e)=>{ state.lineSpacing = parseFloat(e.target.value) || 1.95; saveState(); applyPreferences(); });
  if(el.resetPrefs) el.resetPrefs.addEventListener('click', ()=>{ resetPrefs(); closeSettings(); });

  // scroll -> progress
  if(el.contentArea) el.contentArea.addEventListener('scroll', updateProgress);
  window.addEventListener('resize', updateProgress);

  // Keyboard: Left/Right arrows for navigation, +/- for font size, t for theme, Esc to close modals
